...
const postBookHandler = (request, h) => {
  try {
    ...
    if (payload.name === undefined) {
      ...
      return h.response(failResponse({ responseMessage: message, withData: false })).code(400);
    }
    ...
    return h.response(successResponse({ responseMessage: "Buku berhasil ditambahkan", responseData: { bookId: newBook.id } })).code(201);
  } catch (error) {
...
const getBookHandler = (request, h) => {
  ...
  if (bookIdParam !== undefined) {
    if (bookById === undefined) {
      ...
      return h.response(failResponse({ responseMessage: message })).code(404);
    }
  ...
  return h.response(successResponse({ responseData: { books: finalBooksResult } })).code(200);
};
...
const putBookHandler = (request, h) => {
  ...
  if (searchedBook === undefined) {
    ...
    return h.response(failResponse({ responseMessage: message, withData: false })).code(404);
  }

  searchedBook.updateBook(payload);
  return h.response(successResponse({ responseMessage: "Buku berhasil diperbarui" })).code(200);
};
...
const deleteBookHandler = (request, h) => {
  ...
  return h.response(successResponse({ responseMessage: "Buku berhasil dihapus" })).code(200);
};
